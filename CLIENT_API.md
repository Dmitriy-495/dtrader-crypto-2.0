...
| Канал        | Описание                                                      |
| ------------ | ------------------------------------------------------------- |
| `logs`       | Логи сервера (info, warn, error, debug, success)              |
| `ticks`      | Тиковые данные (каждая сделка)                                |
| `orderbook`  | Обновления Order Book                                         |
| `balance`    | Обновления баланса                                            |
| `indicators` | Данные индикаторов (tick_speed, orderbook_pressure, ema, rsi) |
...
Логи от сервера в реальном времени.
...
## 🔗 Состояние связи с биржей

Для мониторинга состояния связи с Gate.io используйте канал `logs` и отслеживайте следующие сообщения:
...
### Пример 1: Подключение и подписка на индикаторы
...
### Пример 2: Мониторинг состояния соединения
...
ws.on("message", (data) => {
  ...
  if (message.type === "log") {
    // Фильтруем только сообщения о ping-pong
    if (message.message.includes("PONG")) {
      ...
      if (latencyMatch) {
        ...
        console.log(`Задержка до биржи: ${latency}ms`);
...
- Подписывайтесь только на нужные каналы
- Для торговых ботов: `indicators`, `ticks`, `balance`
- Для мониторинга: `logs`, `orderbook`
- Для разработки/отладки: все каналы

### 2. Обработка отключений

- Реализуйте автоматическое переподключение
- При переподключении заново подписывайтесь на каналы
